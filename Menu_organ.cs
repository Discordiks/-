...
namespace Проект
{
    public partial class Menu_organ : Form
    {
        ...
        private void name_champ_combo_SelectedIndexChanged(object sender, EventArgs e) //метод открытия новой формы для работы с выбранным чемпионатом
        {
            this.connection.Open();
            User_info.name_champ = name_champ_combo.Text;
            SQLiteDataAdapter adapter = new SQLiteDataAdapter(); //поиск и запись id только что добавленного чемпионата
            DataTable table = new DataTable();
            SQLiteCommand command = new SQLiteCommand("SELECT * FROM competitions WHERE `title` = @l1", this.connection);
            string l1 = name_champ_combo.Text;
            command.Parameters.AddWithValue("@l1", l1);
            adapter.SelectCommand = command;
            adapter.Fill(table);
            SQLiteDataReader reader = command.ExecuteReader();
            while (reader.Read())
            {
                User_info.id_champ_combo = $"{reader["id"]}";
                break;
            }
            reader.Close();
            this.connection.Close();
            f2 = new Setting_champ();
            f2.Show();
            //this.Hide();
